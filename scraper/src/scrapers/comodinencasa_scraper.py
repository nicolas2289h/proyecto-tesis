...
class ComodinEncasaScraper(BaseScraper):
    """
    Scraper concreto para Comodín en Casa
    """
    ...
    def parse(self, html: str, *args, **kwargs) -> Dict[str, Any]:
        ...
        if not precio:
            # Buscar precio de venta (el en negro, no el tachado)
            # Primero buscamos el precio con el símbolo $ y el formato con coma
            selectores_precio = [
                "[class*='selling-price']",
                "[class*='sellingPrice']",
            ...
            for selector in selectores_precio:
                ...
                for price_elem in price_elems:
                    if price_elem:
                        ...
                        # Filtramos para que sea un precio válido
        ...
        if not url_imagen:
            ...
            for img_elem in img_elems:
                ...
                # Buscamos imágenes que contengan "producto", "product", o sean más grandes (generalmente la principal)
                if img_url and ("product" in img_url.lower() or "imagen" in img_url.lower()):
                    if not img_url.startswith("http"):
                        img_url = urljoin("https://www.comodinencasa.com.ar", img_url)
                    url_imagen = img_url
                    break
    ...
    async def search_and_scrape(self, page: Page, search_term: str, base_url: str = "https://www.comodinencasa.com.ar") -> List[Dict[str, Any]]:
        """
        ...
        while True:
            ...
            for link in all_links:
                href = link.get("href", "")
                # Filtramos para encontrar enlaces de productos (normalmente terminan en /p o tienen /p/)
        ...
        for i, product_url in enumerate(sorted(all_product_urls)):
            try:
                ...
                # Solo agregamos si tenemos al menos el nombre
