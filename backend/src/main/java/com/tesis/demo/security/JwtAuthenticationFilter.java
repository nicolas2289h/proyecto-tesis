...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            userEmail = jwtService.extractClaim(jwt, claims -> claims.getSubject());
        ...
        if (userEmail != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (usuarioOpt.isPresent() && jwtService.isTokenValid(jwt, userEmail)) {
                var claimsRoles = jwtService.extractClaim(jwt, claims -> claims.get("roles", List.class));
                List<SimpleGrantedAuthority> authorities = List.of();
                if (claimsRoles instanceof List<?> raw) {
